...
const handleSubmit = async () => {
  ...
  if (validateForm()) {
    ...
    try {
      let post = await axios.patch(`http://127.0.0.1:8000/api/posts/${postId}`, { ...form })
...
onMounted(async () => {
  ...
  const post = await axios.get(`http://127.0.0.1:8000/api/posts/${postId}`)
